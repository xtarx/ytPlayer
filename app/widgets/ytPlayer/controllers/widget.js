...
exports.playLiveLink = function(streamUrl) {
	exports.isPlaying = true;
	playVideo(streamUrl);
};
...
function getVideo(id) {
	...
	client.onload = function() {
		/*
		 var Settings = Titanium.Filesystem.getFile(Titanium.Filesystem.tempDirectory);
		 var newFile = Titanium.Filesystem.getFile(Settings.nativePath, 'm.txt');
		 newFile.createFile();
		 if (newFile.exists()) {
		 newFile.write(this.responseText.substring(4, this.responseText.length));
		 }

		 Ti.API.info('path:' + Settings.nativePath);
		...
		var player_data = response.content.player_data;
		var isHighQuality = player_data['fmt_stream_map'] != null;
		var streamUrl = isHighQuality ? player_data['fmt_stream_map'][0].url : player_data.stream_url;
		if (!isHighQuality) {
